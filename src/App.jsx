...
// ─── Sidebar ────────────────────────────────────────────────────────────────────
function Sidebar({ activeTab, setActiveTab, role, isOpen }) {
...
function StudentsManager() {
  ...
  const handleAdd = async () => {
    ...
    const { data: school } = await supabase.from("schools").select("id").limit(1).single();
    const { data: authData, error: authError } = await supabase.auth.signUp({ email: form.email, password: form.password, options: { data: { full_name: form.full_name } } });
    ...
    if (userId) {
      await supabase.from("profiles").upsert({
        ...
        role: "student", phone: form.phone, school_id: school?.id, is_approved: true, class_id: form.class_id || null,
      });
      if (form.class_id) {
...
function AppShell() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "dashboard":
        if (role === "admin") return <AdminDashboard setActiveTab={handleTabChange} />;
        if (role === "teacher") return <TeacherDashboard setActiveTab={handleTabChange} />;
        if (role === "student") return <StudentDashboard setActiveTab={handleTabChange} />;
        if (role === "parent") return <ParentDashboard setActiveTab={handleTabChange} />;
        return null;
      ...
      case "quiz": return <QuizModule role={role} profile={profile} />;
      case "homework": return <HomeworkModule role={role} profile={profile} />;
      case "notes": return <NotesModule user={profile} profile={profile} />;
      case "attendance": return <AttendanceModule user={profile} profile={profile} />;
      case "timetable": return <TimetableModule user={profile} profile={profile} />;
      case "fees": return <FeesModule user={user} profile={profile} schoolId={profile?.school_id} />;
      case "results": return <ResultsModule user={user} profile={profile} schoolId={profile?.school_id} />;
      case "library": return <LibraryModule user={user} profile={profile} schoolId={profile?.school_id} />;
      case "meetings": return <MeetingsModule user={user} profile={profile} schoolId={profile?.school_id} />;
